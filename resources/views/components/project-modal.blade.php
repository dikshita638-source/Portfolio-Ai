...
@foreach($projects as $index => $project)
    <div id="project-modal-{{ $index }}"
         class="fixed inset-0 z-[200] hidden items-center justify-center p-4"
         onclick="if(event.target === this) closeProjectModal({{ $index }})">
        <div class="absolute inset-0 bg-dark-950/80 backdrop-blur-sm" onclick="closeProjectModal({{ $index }})"></div>
        ...
        <div class="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto glass-card rounded-3xl border border-white/10 p-8 transform transition-all duration-300">
            {{-- Close Button --}}
            <button onclick="closeProjectModal({{ $index }})"
            ...
            <p class="text-white/50 text-sm leading-relaxed mb-6">{{ $project['long_description'] }}</p>
